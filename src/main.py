...
from src.functions.dev_mode import DevJump, DevSkip
...
def main():
    ...
    while running:
        ...
        try:
            ...
            if state == "START_STEP":
                start_step()
                # Em DEV o operador escolhe onde entrar no fluxo; ESC no menu segue
                # do comeco, igual a PROD.
                state = dev_mode.pick_step() or "SCREEN_STEP"
                continue
            ...
            if state == "KEYBOARD_STEP":
                ...
                if jack_detection_available():
                    draw_text(["Conecte o headphone..."])
                    connected = headphone_connected()
                else:
                    # Sem deteccao de jack o polling ficaria esperando para sempre
                ...
                if connected:
                    ...
                    time.sleep(0.5)
                elif not jack_detection_available():
            ...
            # Aprovacao manual: registra e pula para o proximo passo, sem
            # tocar no estado interno do passo abandonado.
            state = dev_mode.approve(state)
            step = 0
            waiting_remove = False
            pressed_keys.clear()
            continue
        except DevJump as jump:
            state = jump.state
            step = 0
            waiting_remove = False
            pressed_keys.clear()
